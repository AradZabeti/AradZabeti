...
- `terminal.html` — profile shell with history, Tab completion, Easter eggs and the optional safe cPanel diagnostic bridge.
...
The Terminal can optionally call `cpanel/terminal.php` deployed on a cPanel PHP host. The bridge exposes safe diagnostics such as server status, PHP/OS details, storage, runtime limits, capabilities, web/security posture and health checks.
...
- `php` / `os` / `disk` / `web` / `security`
- `ping`
- `watch 10` / `stopwatch`
- private `user` / `cwd` diagnostics when a server-side key is configured
...
The cPanel bridge uses an allowlist, JSON-only POST handling, a per-IP rate limit, strict CORS for GitHub Pages, size limits and optional private-key protection. The preferred private-key file location is outside `public_html`.
